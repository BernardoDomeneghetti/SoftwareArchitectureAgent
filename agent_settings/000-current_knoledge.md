...
### 2026-08-24 — Semana 2, Dia 1 (LangGraph — topologia do grafo, arestas condicionais)

- Diante do design inicial com um nó `Evaluate` fazendo o grafo voltar e reavaliar sequencialmente (`A -> B -> Evaluate -> C -> Evaluate -> D`), questionado sobre o padrão scatter-gather/`Task.WhenAll` que já usou em C#, **transferiu sozinho** a ideia de disparo paralelo com sincronização pro contexto do LangGraph — concluiu, sem o mentor afirmar, que uma aresta condicional pode devolver uma **lista** de nós-destino (`["B","C"]`), eliminando a necessidade do nó `Evaluate`. *Evidência: aplicação de um padrão de sistemas distribuídos já dominado a uma API nova, apenas a partir de uma pergunta-gancho, sem receber o mecanismo do LangGraph explicado antes.*
- Ao ser questionado se a função condicional de `A` poderia devolver `D` **junto** com `B`/`C` na mesma lista, identificou sozinho a inconsistência: `D` só faz sentido quando **nenhum** dos dois recursos é necessário — nunca em conjunto (senão rodaria antes da busca terminar). *Evidência: raciocinou sobre uma invariante de ordenação/dependência de dados no grafo sem que o mentor apontasse o problema diretamente, só perguntasse "o que aconteceria".*
- Depois de o mentor explicar (a pedido explícito do aluno, que não soube deduzir sozinho) que nós do LangGraph não mutam o `State` recebido — cada nó devolve um `dict` parcial, mesclado pelo grafo, evitando o mesmo bug de "sobrescrita silenciosa" já visto no Dia 1 (chave de dicionário colidindo) — aplicou o conceito de volta ao próprio `State` do projeto e concluiu corretamente, sem ser dito, que não há colisão de chave entre `B` (escreve `related_logs`) e `C` (escreve `rag_response`), e que `inference_result` é responsabilidade exclusiva de `D`. *Evidência: aplicação do mecanismo recém-explicado a um caso concreto do próprio design, checando ownership de campo por campo.*

### 2026-08-24 — Semana 2, Dia 1 (LangGraph + MCP real — `call_logs_node`)

- Diante da pergunta se o grafo deveria usar o atalho in-process do SDK do MCP (`Client(mcp_server_instance)`, achado no docstring) ou reconectar via subprocess+stdio como desenhado no Dia 2, articulou sozinho a razão certa pra manter o caminho mais complexo: o objetivo declarado da Semana 1 foi treinar a mecânica real do protocolo, e o atalho jogaria isso fora. *Evidência: escolha de arquitetura justificada por objetivo pedagógico, não só funcionalidade.*
- Diante do erro real `TypeError: 'tuple' object does not support the asynchronous context manager protocol`, guiado a olhar a linha exata do traceback (dentro do próprio `Client`, chamando `enter_async_context(transport)`), concluiu corretamente que o `Client` esperava um gerenciador de contexto **fechado** (não a tupla já aberta que o código antigo produzia com um `async with stdio_client(...) as transport` prévio). *Evidência: leitura de traceback real para inferir contrato de API, não só reconhecer o sintoma.*
- Ao ver o erro "Failed to parse JSONRPC message" e ser lembrado da regra de framing do Dia 2 (newline-delimited JSON, uma mensagem por linha), reconheceu sozinho e rapidamente que os `print(...)` de debug dentro do `lifespan()` do servidor MCP — escritos no mesmo stdout usado pro protocolo — violavam essa regra, e removeu-os. *Evidência: aplicação direta de uma regra abstrata aprendida meses antes (sem execução real na época) a um bug concreto, ao vivo.*
- Reconheceu a colisão de responsabilidade entre o `Pool` do Postgres (do servidor, já existente) e a conexão do client ao processo do servidor, quando questionado — confirmou que o client só consome o serviço, não duplica a infra do servidor.
