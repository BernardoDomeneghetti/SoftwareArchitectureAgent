...
- **Driver Python decidido:** `asyncpg` (assíncrono nativo) em vez de `psycopg2` — motivo: o SDK do MCP já é assíncrono (starlette/anyio como dependências transitivas), então um driver síncrono bloquearia a thread durante a query. Instalado em 2026-08-13 (`asyncpg==0.31.0`, rodado pelo próprio aluno via `uv add asyncpg`).
- **Correção de API (2026-08-13):** o pacote `mcp>=2.0.0` instalado neste projeto é mais novo do que o conhecimento de referência do mentor sobre o SDK — a classe de alto nível **não** é `FastMCP` em `mcp.server.fastmcp` (nome usado em versões antigas do SDK, e que o mentor citou por engano). Neste pacote, a classe correta é **`MCPServer`**, importada de **`mcp.server.mcpserver`** (junto com `Context`, também deste mesmo submódulo). O padrão de uso (decorator `.tool()`, injeção de `Context` como parâmetro, acesso a estado do `lifespan` via `ctx.request_context.lifespan_context`) permanece igual — só o nome da classe e o caminho do import mudaram. Confirmado lendo os arquivos reais em `src/.venv/Lib/site-packages/mcp/server/mcpserver/`. **Sempre que precisar citar uma API desse SDK, verificar primeiro os arquivos instalados em `src/.venv`, em vez de confiar em conhecimento prévio** — o pacote evolui rápido e pode ter mudado desde o treinamento do modelo.